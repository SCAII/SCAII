...
pub use self::qna::{
    Qna, ClickTarget, ClickTargets, DecisionStep, QnaItem, QuestionText, QuestionType,
    RadioButtons, Text
};
...
impl UserStudyQuestions {
    ...
    pub fn get_questions(&mut self) -> Result<Option<StudyQuestions>, Box<Error>> {
        ...
        let qna = Qna::try_from_file(&questionfile_path)?;

        let study_questions = StudyQuestions {
            user_id: user_id,
            treatment_id: treatment_id,
            study_questions: From::from(qna),
    ...
    pub fn persist_log_entry(&mut self, lfe: LogFileEntry) -> Result<(), Box<Error>> {
        ...
        if lfe.is_last_line {
            ...
            answerfile_path.push(filename);
            if answerfile_path.exists(){
                fs::remove_file(&answerfile_path)?;
            }
