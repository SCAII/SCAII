...
impl ReplaySequencer {
    pub fn new(replay_info: &Vec<ReplayAction>) -> Result<ReplaySequencer, Box<Error>> {
        ...
        if keyframe_indices.len() == 0 {
            ...
            return Err(Box::new(ReplayError::new("No keyframes present in recorded file")));
        }
    ...
    pub fn ff_simple_mode(&mut self, prior_keyframe_index: u32, target : u32) -> Result<Vec<ScaiiPacket>, Box<Error>> {
        let mut result : Vec<ScaiiPacket> = Vec::new();
        let mut index = prior_keyframe_index;
        while index <= target {
            let spkt = self.scaii_pkts[index as usize].clone();
            if index == 0 {
                println!("   ...pushed keyframe at index 0");
            }
            else {
                println!("   ...pushed action at index {}", index);
            }
            result.push(spkt);
            index = index + 1;
            println!("   ...incr index to {:?}", index);
        }
        self.index = index;
    ...
    pub fn ff_prepend_mode(&mut self, prior_keyframe_index: u32, target : u32) -> Result<Vec<ScaiiPacket>, Box<Error>> {
        let mut result : Vec<ScaiiPacket> = Vec::new();
        let key_frame_pkt = self.keyframe_map.get(&prior_keyframe_index).unwrap().clone();
    ...
    pub fn get_prior_key_frame_index(&mut self, starting_point : u32) -> u32 {
        ...
        println!("-look backward through the list of keframe indices starting at {}",cur_index);
        while cur_index >= 1 {
            ...
            if value <= starting_point {
                println!("determined that {} is the index prior to {}", value, starting_point);
                return value;
            }
        }
        0
